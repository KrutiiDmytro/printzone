...
- **`docker update --restart` не переживає деплой.** Під час інциденту 05.08 політики
  шести контейнерів ядра виставили руками; за п'ять днів деплої перестворили `app-nginx-1`
  і `app-php-1`, і ті знову стали `restart: no` — тобто рівно ті два, від яких залежить
  сайт. Ребут поклав би магазин без шансу піднятися. Політика мусить бути в compose-файлі,
  інакше вона живе до першого перестворення контейнера.
  - Перевірка перед будь-яким ребутом чи апгрейдом docker:
    `docker inspect -f '{{.Name}} {{.HostConfig.RestartPolicy.Name}}' $(docker ps -aq) | grep -v unless-stopped`
    має бути порожньою.
  - Prod-only політику класти в `compose.prod.yaml` (як у всіх 9 винесених сервісів), не
    в базовий `compose.yaml` — інакше локальний dev-стек почне стартувати з Docker Desktop.
